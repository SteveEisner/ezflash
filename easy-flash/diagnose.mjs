...
export function parseDiagnosticText(text="") {
	...
	return {raw,state:rescue?"rescue":raw.trim()?"telemetry":"unsupported",rescue,ledsAbsent:rescue,networkAbsent:rescue,target,targetIdentity:target?"exact-report":"unknown",observedTarget,chip:observedChip,button,buttonProblem:button?.toUpperCase()==="STUCK_BUTTON",buttonDiagnostics:["available","inactive","healthy"].includes(normalizedButton)?normalizedButton:normalizedButton?.startsWith("disabled")?"disabled":button?"problem":null};
}
...
export function diagnoseSummary(diagnostic) {
	...
	return "No supported diagnostic telemetry was found. This does not prove the controller is healthy or faulty.";
}
...
export function createDiagnoseRuntime({serial=globalThis.navigator?.serial,baudRate=DEFAULT_BAUD_RATE,timeoutMs=READ_WINDOW_MS,maxBytes=MAX_BYTES}={}) {
	async function inspect({onText=()=>{},onStatus=()=>{}}={}) {
		...
		try {
			...
			if(!reader)return {...parseDiagnosticText(""),portInfo};const rawBytes=new Uint8Array(maxBytes),deadline=Date.now()+timeoutMs;
			...
			const parsed=parseDiagnosticText(text);onText(parsed.raw);return {...parsed,portInfo,bytesCaptured:bytesRead,readError:readError?.message||null};

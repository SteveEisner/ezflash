...
test("launch is connect-first with the controller list always expanded", async () => {
	...
	assert.match(html,/<details id="advancedDetails">/); assert.match(html,/Buy\. Build\. <em>Rave\.<\/em>/); assert.match(html,/id="recommendation"/); assert.match(html,/Need a controller/i); assert.match(html,/href="https:\/\/dig2go\.info\/" target="_blank" rel="noopener noreferrer"/); assert.match(html,/ready-made starting point for a WLEDTubes Tube/); assert.match(html,/Why Dig2Go\?/); assert.match(html,/Steve beta-tested early Dig2Go hardware while WLEDTubes was taking shape/); assert.doesNotMatch(html,/Make your own Tube|Bring the flock home|I already have a controller|Back to intro|landingHero|landing-actions/); assert.match(html,/<summary>Advanced updates<\/summary>/); assert.match(html,/Coming soon<\/strong><p>OTA and nearby-device updates will live here\. USB installation is available above\.<\/p>/); assert.doesNotMatch(html,/Targets|artifact|P2P.*button|button.*P2P/);

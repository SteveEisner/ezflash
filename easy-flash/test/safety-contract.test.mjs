...
test("prepared token, exact port, physical model, and immediate chip recheck gate the pre-write boundary",async()=>{
	...
	const connected=await fixture.runtime.connectController({variant,onStatus(){}});const evidence=await fixture.runtime.detectController({sessionToken:connected.token,port:connected.port,variant,onStatus(){}});const base={variant,artifact,sessionToken:evidence.token,port:evidence.port,onStatus(){},onProgress(){},beforeWrite:fixture.mark};

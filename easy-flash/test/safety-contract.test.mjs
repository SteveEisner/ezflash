...
test("prepared token, exact port, target action, and immediate chip recheck gate the pre-write boundary",async()=>{
	const manifest=await loadFirmwareManifest(),variant=structuredClone(manifest.variants[0]);variant.id="quinled-dig2go";variant.target.printedModel=variant.target.board;const artifact={...variant.artifacts[0],url:"https://flash.test/releases/test/firmware/merged.bin"},fixture=runtimeFixture();
...
test("wrong target/model, swapped observations, and serial disconnect invalidate safely",async()=>{
	const manifest=await loadFirmwareManifest(),variant=structuredClone(manifest.variants[0]);variant.id="quinled-dig2go";variant.target.printedModel=variant.target.board;const artifact={...variant.artifacts[0],url:"https://flash.test/releases/test/firmware/merged.bin"};

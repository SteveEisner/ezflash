...
test("Status renders shared release evidence once and all three target artifacts",()=>{
	...
	assert.equal(nodes[1].textContent,"Supported firmware");
	assert.equal(nodes[2].children.length,3);
	for(const [,board] of targets){const card=nodes[2].children.find(node=>node.children?.[0]?.textContent?.includes(board));assert.ok(card,`missing ${board}`);}

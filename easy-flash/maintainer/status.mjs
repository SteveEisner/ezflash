...
export function renderStatus(container,{current,manifest,receipt}){const p=manifest.provenance||{},variants=manifest.variants||[];const shared=section('Release evidence',[['Release',current.releaseId],['Snapshot generated',current.generatedAt],['Evidence mode',p.mode],['Build time',receipt?.timestamps?.builtAt||receipt?.builtAt],['Deployment time',receipt?.timestamps?.deployedAt],['Source repository',variants[0]?.source?.repository],['Full commit',variants[0]?.source?.commit],['GitHub Actions run',receipt?.ci?.runUrl||receipt?.ci?.runId]]);const heading=document.createElement('h2');heading.textContent='Supported firmware';const list=document.createElement('div');list.className='device-list';variants.forEach(v=>list.append(device(v)));if(list.addEventListener)list.addEventListener('toggle',event=>{if(!event.target.open)return;list.querySelectorAll('details').forEach(item=>{if(item!==event.target)item.open=false})},true);container.replaceChildren(shared,heading,list)}

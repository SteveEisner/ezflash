...
export function diagnosePresentation(d={}){
 if(d.state==="rescue")return {tone:"red",label:"Recovery mode reported",summary:"The USB device reported recovery mode.",next:"Confirm the exact target before using Flash; Diagnose does not start recovery.",action:"Check another USB device",recovery:true};
 if(d.buttonProblem)return {tone:"red",label:"Stuck button reported",summary:"The USB device reported a stuck button.",next:"Follow the controller's button recovery guidance before checking again.",action:"Check another USB device",recovery:true};
 if(d.state==="malformed"||d.error)return {tone:"red",label:"Report needs attention",summary:"The USB device sent data, but the report contained conflicting or malformed diagnostic information.",next:"Review the technical details before checking again.",action:"Check another USB device"};
 if(d.state==="telemetry"&&d.target&&d.chip&&d.buttonDiagnostics!=="stuck_button")return {tone:"green",label:"Looks healthy",summary:"The USB device sent a valid diagnostic report and did not report a problem.",next:"",action:"Check another USB device"};
 if((d.bytesCaptured??0)>0)return {tone:"yellow",label:"Report not recognized",summary:"The USB device sent data, but Easy Flash could not read it as a diagnostic report.",next:"",action:"Choose USB device again"};
 return {tone:"yellow",label:"No diagnostic report",summary:"Easy Flash opened the USB device, but it did not send a diagnostic report. This can happen when the installed firmware does not support Diagnose.",next:"",action:"Choose USB device again"};
}

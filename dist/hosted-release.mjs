...
export function validateHostedCatalog(manifest,releaseId,baseUrl) {
	...
	const catalog=manifest.variants.map(variant=>{
		...
		const candidates=variant.artifacts?.filter(a=>a.transport==="usb"&&a.kind==="complete-merged-image")||[];if(candidates.length!==1)throw Error("Each target must select exactly one ordinary merged USB image");
		...
		const otaCandidates=variant.artifacts?.filter(a=>a.transport==="ota"&&a.kind==="application-image")||[];if(otaCandidates.length>1)throw Error("Each target may select only one ordinary laptop OTA image");const ota=otaCandidates[0]?{...otaCandidates[0]}:null,otaFile=ota?.path?.slice(prefix.length);if(ota&&(!ota.path?.startsWith(prefix)||!FILE_NAME.test(otaFile||"")||!Number.isSafeInteger(ota.sizeBytes)||ota.sizeBytes<=0||!/^[a-f0-9]{64}$/.test(ota.sha256)))throw Error("The laptop OTA artifact contract is invalid");
		const propagating=variant.artifacts?.filter(a=>a.transport==="usb-propagating"&&a.kind==="complete-merged-image")||[];if(propagating.length>1||propagating.length&&variant.id!=="quinled-dig2go")throw Error("Only one Dig2Go propagating image is allowed");const prop=propagating[0];if(prop){const propFile=prop.path?.slice(prefix.length);if(!prop.path?.startsWith(prefix)||!FILE_NAME.test(propFile||"")||!Number.isSafeInteger(prop.sizeBytes)||prop.sizeBytes<=0||!/^[a-f0-9]{64}$/.test(prop.sha256)||prop.offset!==0)throw Error("The propagating artifact contract is invalid");}
		return {variant,artifact:{...artifact,url:exactSameOriginUrl(artifact.path,baseUrl,"The artifact path").href},otaArtifact:ota?{...ota,url:exactSameOriginUrl(ota.path,baseUrl,"The OTA artifact path").href}:null,propagatingArtifact:prop?{...prop,url:exactSameOriginUrl(prop.path,baseUrl,"The propagating artifact path").href}:null};
	});

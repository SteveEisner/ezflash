...
if(fixture){
	contract={schemaVersion:1,targets:lock.targets.map((item,index)=>({id:item.targetId,hardwareFamily:item.targetId,chipFamily:['ESP32','ESP32-C3','ESP32-S3'][index],board:item.printedModel,flashMode:index===2?'keep':'dio',flashSizeBytes:index===2?16777216:4194304,compiledProfile:{environment:item.environment},partition:{csvPath:`${item.targetId}.csv`,sha256:'0'.repeat(64),otaSlots:[{offset:65536,sizeBytes:index===2?6291456:1572864}]}})),artifacts:lock.targets.map(item=>({targetId:item.targetId,kind:'application-image',transport:'ota',wledBaseVersion:'16.0.1',tubesRelease:'14'}))};
}else{
...
for(const locked of lock.targets){
	...
	const usb={...(await evidence(usbPath)),path:`targets/${target.id}/merged.bin`,kind:'complete-merged-image',transport:'usb',writeOffset:0,components};const ota={...(await evidence(otaPath)),path:`targets/${target.id}/firmware.bin`,kind:'application-image',transport:'ota',buildOffset:target.partition.otaSlots[0].offset};const partition={...(await evidence(partitionPath)),path:`targets/${target.id}/${basename(partitionPath)}`,csvPath:target.partition.csvPath};const applicationContract=contract.artifacts.find(item=>item.targetId===target.id&&item.kind==='application-image'&&item.transport==='ota');if(!applicationContract)throw Error(`application identity missing for ${target.id}`);const bootIdentity={version:1,target:target.id,source:lock.commit,release:applicationContract.wledBaseVersion,tubes:Number(applicationContract.tubesRelease)};targets.push({targetId:target.id,environment:locked.environment,printedModel:locked.printedModel,bootIdentity,contractTarget:target,partition,artifacts:{usb,ota}});
}
